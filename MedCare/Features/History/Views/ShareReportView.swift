...
struct ShareReportView: View {
    ...
    private func generateWhatsAppText() -> String {
        let rangeDays: Int
        switch selectedRange {
        case .sevenDays: rangeDays = 7
        case .fourteenDays: rangeDays = 14
        case .thirtyDays: rangeDays = 30
        case .custom:
            rangeDays = Calendar.current.dateComponents([.day], from: customStartDate, to: customEndDate).day ?? 0
        }

        var lines: [String] = []
        lines.append("\u{1F4CB} MedCare Health Report")
        lines.append("\u{1F464} \(profile.name) | \u{1F4C5} Last \(rangeDays) days")
        lines.append("")
        lines.append("\u{1F48A} Adherence: \(String(format: "%.0f", adherencePercent))%")
        lines.append("\u{1F4CA} Health Score: \(healthScore.total)/100")
        lines.append("")
        lines.append("\u{1F48A} Medicines:")

        // Per-medicine adherence
        for medicine in allMedicines.filter({ $0.isActive }) {
            let medLogs = medicine.doseLogs.filter { dateRange.contains($0.scheduledTime) }
            let total = medLogs.count
            let taken = medLogs.filter { $0.status == .taken }.count
            let pct = total > 0 ? Int(Double(taken) / Double(total) * 100) : 0
            let icon = pct >= 85 ? "\u{2705}" : "\u{26A0}\u{FE0F}"
            lines.append("\(icon) \(medicine.brandName) - \(pct)%")
        }

        lines.append("")
        lines.append("Generated by MedCare App")
